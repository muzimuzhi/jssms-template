% !TeX encoding = GBK
\documentclass{jssms}
%建议使用此模版前登陆www.ctex.org/CTeXDownload下载Latex软件.
%此模板需调用宏包jssms.cls, 请确保此宏包与tex文件在同一文件夹下.
%请使用CCT&LaTex编译.

%%
%% 使能通过 pdflatex 编译
%%
\AtBeginDvi{\input{zhwinfonts}}

\numberwithin{equation}{section}
% 公式编号会随着Section而变动,如(1.1),(1.2),(2.1)...,如果不便使用,您可以将这个命令删去
%--------------文内自定义命令----------------
\usepackage[pass, showframe]{geometry}

\def\hat{\widehat}
\def\tilde{\widetilde}
\def\bar{\overline}
\def\epsilon{\varepsilon}
\def\dd{{\rm d}}
\def\ii{{\rm i}}
\def\ee{{\rm e}}
\def\dint{\displaystyle\int}
\def\vsp{\vspace{1mm}}
\def\q{\quad} \def\qq{\qquad}
\def\la {\langle}              \def\ra {\rangle}
\def\*{$\!\!^{^{^{\displaystyle *}}}$}
%**************************************************
\def\jssmszi{\zihao{10}\ziju{0.135}}
\parindent=2\ccwd
\def\rmn#1{\romannumeral #1}
\def\Rmn#1{\expandafter\uppercase\expandafter{\romannumeral #1}}
%---------------------------------------
%--------------文内自定义命令结束------------

%--------------作者自定义命令----------------



%--------------作者自定义命令结束------------

%*************************************************
%***************************************

%-------------正文开始------------------

\begin{document}

\thispagestyle{empty}

\Volume{20xx}{x}{x}{x}               % 年、月、卷、期
\PageNum{1}                                  % 起始页码
\PaperID{0583-1431(20xx)0x-0xxx-0x}   % 文章编号
\DocumentCode{A}                              % 文献标识码
\EditorNote{*基金项目.}%如 *国家自然科学基金(10171074)资助课题.  如没有基金项目请删去此行


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%  作者从下面开始填写各项内容,本刊的标点符号均使用英文标点,您可以在编译前设置标点形式

\EditorNote{收稿日期: 200x-xx-xx, 收到修改稿日期: 200x-xx-xx.}  % 脚注
\EditorNote{编委:  }% 脚注，作者不必填写。

\TitleMark{作者姓名: 论文标题缩写}
% 页眉, 如有多位作者,请写为： 第一作者姓名等: 论文标题缩写

\begin{titlepage}

\Title{论文标题\*}

\Author{姓名1$^{1}$\q\q 姓名2$^{2}$\q\q 姓名3$^{3}$}%例如: 张\ 小\ 乙
 {(1. 作者单位,\, 城市 邮编; 2. 作者单位,\, 城市 邮编; 3. 作者单位,\, 城市 邮编)} % 例如 (中国科学院数学与系统科学研究院,\ 北京 100190)



\Abstract{摘要内容}

\Keywords{关键词1, 关键词2, 关键词3, $\cdots$.}%甲, 乙, 丙, 丁.

\MRClass{MR分类号1, MR分类号2}%需作者自己提供

\ETitle{TITLE} %英文标题名称全大写

\EAuthor{% 作者姓名的拼音,姓在前,名在后,
  \uppercase{XING} Ming1$^{1}$\qquad
  \uppercase{XING} Ming2$^{2}$\qquad
  \uppercase{XING} Ming$^{3}$%
}{%
  \small
  (1. \textit{Author's Address,\, City} Postcode; 
   2. \textit{Author's Address,\, City} Postcode; 
   3. \textit{Author's Address,\, City} Postcode;)%
}
% 如 (\textir{Academy of Mathematics and Systems Science, Chinese Academy of Sciences, Beijing} 100190)

%顺序相邻的几个作者若单位相同，合并在一起排版。
%\EAuthor{\uppercase{XING} Ming\q \q \uppercase{XING} Ming}
%{\small(Author's Address)}

\EAbstract{In this paper, we $\cdots$}
注意:英文摘要需将文中研究结果和结论尽量详细阐述,使不懂中文的学者也能了解到文章的研究成果.

\EKeywords{Keyword1, Keyword2, Keyword3, $\cdots$.}

\end{titlepage}



\section{引\ \ 言}

Bustos和Concha\supercite{ref1}讨论了$\cdots$,其它结论见文 \cite{ref2,ref3,ref4}.

公式示例\supercite{ref2,ref3,ref5,ref6,ref7}
\begin{align}\label{eq:1.1}
  a &= b \notag\\
    &= c \notag\\
    &= d.
\end{align}
其中由式\eqref{eq:1.1}, 我们可以得到\FootNote{脚注的命令格式.}
% 也可以使用(\ref{eq:1.1})来引用公式编号
\begin{align*}
  a = b.
\end{align*}

{\HT 定义 1.1}\ \   内容

{\HT 引理 1.2}\ \   内容

{\FS 证}\ \   内容

{\KS 注 1.3}\ \   内容

注意文中的标点符号应使用英文半角状态下的格式,例如: , . : ; ? ( ) ``''等; 使用的大写希腊字母统一要用斜体标示,例如: $\varPhi,\varLambda,\varTheta,\varOmega,\cdots$; 文章中请使用1), 2), 3),
$\cdots$或 (i), (ii), (iii), $\cdots$ 或 (h), (i), (g), $\cdots$表示分类符号; 公式中的微分符号$d$要正体,例: $\int f(x)\dd x$,$\dd y(t)=t^2+1$; 常数$e$字体为正体,例: $\mathrm{e}$或$\ee$,除此之外,公式序列要先列举两项,再使用省略号 $\cdots$, 例: $\{f(x_i)$, $i=1,2,\cdots,n\}$, $y_1<y_2<\cdots<y_n$,$A=A_1+A_2+\cdots+A_m$,  $\cdots$.

\section{主要定理及证明}

{\HT 定理 2.1}\ \   内容

{\FS 证}\ \   内容

\subsection{表格示例}

\begin{table}[H]
  \small\centering  
  \caption{标题}
  \centerline{(Table 1\ \ Title)}%%表中文标题下附英文标题翻译。
  \vskip 1mm 
  \begin{tabular}{cccccc}
    \hline
    \multirow{2}{*}{$\rho$} 
    & \multicolumn{2}{c}{AR(1)} 
    & \multicolumn{2}{c}{MA(1)} \\ \cmidrule(r){2-3}\cmidrule(r){4-5}
        & $\theta_{0}$ & $\beta_{0}$ & $\theta_{0}$ & $\beta_{0}$   \\ \hline
    0.6 & 0.0037 & 2.1503 & 0.0071 & 2.1247 \\
    0.1 & 0.0034 & 1.8573 & 0.0034 & 1.7147 \\
    0.0 & 0.0031 & 1.8490 & 0.0030 & 1.6400 \\
    0.1 & 0.0033 & 2.0329 & 0.0035 & 2.1271 \\ \hline
  \end{tabular}
\end{table}
%\begin{table} \end{table}环境也可，但图标位置会浮动.

\subsection{图示例}
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.42]{1.eps}
  \caption{标题}
  \centerline{\small (Figure 1\ \ Title)}%%图中文标题下附英文标题翻译。}
\end{figure}
其中[scale=0.42]的参数可以调节大小.
%\begin{figure} \end{figure}环境也可，但图位置会浮动.

{\KS 注 2.2}\ \
注意文中提供的图要求为清晰的矢量图(eps格式或pdf格式),图为Times New
Roman Regular字体,图中变量符号为Times New Roman
 Italic字体(可使用Adobe Illustrator软件实现),图中所使用的数学符号要与文中格式保持一致.

\subsection{公式示例}

%----------------------------------
%--文中公式需注意：
%--文中的复杂公式应独立成行,
%--公式格式需规整,
%--公式中括号不应小于其中内容,
%--公式后应有标点符号.
%----------------------------------

\begin{align}\label{E:2.1}
    AAAAAAAAAA &= BBBBBBBBBBB      \notag \\
               &\quad + CCCCCCCCCC \notag \\
               &= DDDDDDDDDDDDD.
\end{align}
%公式太长时，换行注意相应符号对齐格式.

\begin{align}\label{E:2.2}
  AAAAAA &= (A+B) - \big(B^{-2^A}+1\big)D - C, \\
  BBBB   &= \bigg\{\frac{A}{B} + \dfrac{B^2-C+\frac{A}{D}}{CC+AD}\bigg\} 
            - CCCC \notag\\ 
         &\quad + DDDDD, \\
  DD     &= \varOmega A + \varPhi_{2}.
\end{align}

%多行公式时注意左对齐格式;注意使用合适大小的括号.
\begin{align}\label{E:2.3}
  & A_{1}=B^{\rm T},\quad A_{2}=C, \quad A_{3}=D,\\
  % 注意多个公式之间要用\quad隔开. 矩阵向量的转置符号T要正体.
  & AA = \begin{cases}
         BBB, & C=DD, \\
         0,   & \text{其他}.
       \end{cases}
\end{align}
% 注意各列要左对齐.

%以下为公式排版式例.
\begin{equation}
  \begin{split}
    \|\tilde{u}^{y}(\cdot,t)\|_{L^{1}}
    &\leq \ee^{\bar{\beta}t}\|u_{0}\|_{L^{1}}
    +\int_{0}^{t}\ee^{\bar{\beta}(t-s)}\bigg\|
    \dfrac{f(\cdot,s)}{y(s)}\bigg\|_{L^{1}}\dd s \\
    &\leq \ee^{\bar{\beta}t}\|u_{0}\|_{L^{1}}
    +\int_{0}^{t}\ee^{\bar{\beta}(t-s)}\dfrac{\|f(\cdot,s)
    \|_{L^{1}}}{y(s)}\dd s \\
    &\leq \ee^{\bar{\beta}T}\bigg(\|u_{0}\|_{L^{1}}+\dfrac{\|f\|_{L^{1}(Q)}}
    {\delta}\bigg)\doteq r_{0}.
  \end{split}
\end{equation}

%另外注意在\begin\{array\}\end\{array\}数组环境下的
%公式中,\sum,\int, \prod, \frac,\cdots前都要添加displaystyle命令.


\vskip 1cm {
%%特别提醒:参考文献中若为中文文献，请后附英文翻译。
\begin{reference} %%{作者名, 文题, 刊, 书名, 年, 卷(期): 起止页码.}

\bibitem{ref1}列举几类参考文献的排版格式 (Layout the formats of several
references).

\bibitem{ref2} Bustos M C, Concha F. On the construction of global
weak solutions in the
kynch theory of sedimentation. {\it Math. Methods in the Appl.
Sci.}, 1988,  {\bf 10}(3): 245--264.%注:英文文献期刊排版格式

\bibitem{ref3} 赵群依, 刘顺兰,王江柱. 生成$M$序列的一种新的算法. 计算机安全,
2007, (11): 11--13. \\
(Zhao Q Y, Liu S L, Wang J Z. A new algorithm of $M$ sequences
generation. {\it Network $\&$ Computer Security}, 2007, (11):
11--13.)%注:中文参考文献需附对应英文翻译，且中文文章杂志名称不用斜体。


\bibitem{ref4} Courant R,  Friedrich K F. Supersonic Flows and
Shock Waves. New York:
Wiley-Interscience, 1948.%注:英文文献书排版格式

\bibitem{ref5} 冯克勤, 刘凤梅. 代数与通信. 北京: 高等教育出版社, 2005.
\\(Feng K Q, Liu F M. Algebra and Communication. Beijing: Higher
Education Press, 2005.)%注:中文文献书排版格式

\bibitem{ref6} Chen W H, Wei C G,  Lu X M. Mean square exponential
 stability of uncertain linear impulsive stochastic
  systems with Markovian switching.  Chinese Control and Decision
  Conference, 2013, 22--34.%注:会议论文排版格式

\bibitem{ref7} 李骥泽, 廖正录, 申世芳, 等. 用ARIMA 模型对
我国猪肉价格的走势分析. 猪业科学, 2013, (6): 126--129.\\
(Li J Z, Liao Z L, Shen S F, et al. The analysis of the pork price
change trend in China by the ARIMA model. {\it Swine Industry
Science}, 2013, (6): 126--129.)%注:超过三个作者的文献排版格式

\bibitem{ref8} 赵闯. 结合灰色理论的BP 神经网络猪肉价格预测的建模与改进研究.
硕士论文.吉林大学, 吉林,
2010.\\
(Zhao C. The modeling and improvement of BP neural network
prediction for the price of pork which combined with gray theory.
Master Thesis. Jilin University, Jilin, 2010.)%注:学位论文排版格式

\bibitem{ref9}Text.
\bibitem{ref10}Text.
\end{reference}

}附录
%请将附录部分放于此处编排。

\normalfont\bfseries eng 中国那我呢\par
\normalfont\rmfamily eng 中国那我呢\par
\normalfont\itshape eng 中国那我呢\par
\normalfont\sffamily eng 中国那我呢\par

\end{document}
